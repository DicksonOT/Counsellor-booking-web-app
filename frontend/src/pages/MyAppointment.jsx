      ...
      {appointments.length == 0
        ...
        : <div className='mx-5 mt-19'>
          ...
          <div className='flex justify-center'>
            {seeMore ? ( <button onClick={() => { setSeeMore(false); setSeeMoreValue(3); }} className='bg-blue-500 text-white text-base rounded-full py-2 px-10 mt-10 cursor-pointer'>
                Show less
              </button>
            ) : (
              <button  onClick={() => { setSeeMore(true); setSeeMoreValue(10); }} disabled={appointments.length < 3} className={`bg-blue-500 text-white text-base rounded-full py-2 px-10 mt-10 ${appointments.length < 3 ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
              >
                Show More
              </button>
            )}
