...
const filteredPrograms = (programs || []).filter(program => {
  const title = program.title?.toLowerCase().trim() || "";
  const instructor = program.instructor?.toLowerCase().trim() || "";
  const category = program.category?.trim() || ""; // Don't convert to lowercase for comparison
  const difficulty = program.difficulty?.trim() || ""; // Don't convert to lowercase for comparison
  const price = program.price?.trim() || ""; // Don't convert to lowercase for comparison
  ...
  const matchesCategory =
    !filterCategory || category === filterCategory; // Direct comparison without case conversion

  const matchesDifficulty =
    !filterDifficulty || difficulty === filterDifficulty; // Direct comparison without case conversion

  const matchesPrice =
    !filterPrice || price === filterPrice; // Direct comparison without case conversion

  return matchesSearch && matchesCategory && matchesDifficulty && matchesPrice;
});

console.log("Programs:", programs);
console.log("Filter states:", { filterCategory, filterDifficulty, filterPrice, searchTerm });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPrograms.map(program => (
            <div key={program._id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow overflow-hidden">
              <div className="relative">
                <img
                  src={program.thumbnail || 'https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=400&h=200&fit=crop'}
                  alt={program.title}
