...
const CounsellorDashboard = () => {
  const { cToken, dashBoard, dashInfo, counsellorInfo } = useContext(CounsellorContext)
  const { formattedDateUniversal, currency } = useContext(AppContext)
  ...
  // Limit appointments to show only the latest 10
  const latestAppointments = dashInfo?.latestAppointments?.slice(0, 10) || []
    ...
    <div className='max-w-8xl mx-auto p-6'>
      ...
      <div className='mb-8'>
        <div className="flex items-baseline gap-2">
          ...
          <h1 className="text-blue-500 text-2xl font-bold">{counsellorInfo.name}!</h1>
        </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8'>
        ...
        <div className='bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 border border-blue-200'>
          <div className='flex items-center justify-between mb-4'>
            ...
            <div className='text-right'>
              <p className='text-2xl font-bold text-blue-700'>{currency} {dashInfo.earnings}</p>
              <p className='text-blue-600 text-sm font-medium'>Total Earnings</p>
            </div>
          </div>
          <div className='flex items-center text-blue-600'>
            <span className='text-xs font-medium bg-blue-200 px-2 py-1 rounded-full'>This Month</span>
          </div>
        </div>
        ...
        <div className='bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-xl shadow-md hover:shadow-lg transition-all duration-300 border border-blue-200'>
          <div className='flex items-center justify-between mb-4'>
            ...
            <div className='text-right'>
              <p className='text-2xl font-bold text-blue-700'>{dashInfo.users}</p>
              <p className='text-blue-600 text-sm font-medium'>Total Clients</p>
            </div>
          </div>
          <div className='flex items-center text-blue-600'>
            <span className='text-xs font-medium bg-blue-200 px-2 py-1 rounded-full'>Active</span>
          </div>
        </div>
      </div>
        ...
        <div className='bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-4 border-b border-gray-200'>
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-sm text-gray-600'>Recent appointments overview (Latest 10)</p>
            </div>
          </div>
        </div>
            ...
            latestAppointments.map((item, index) => (
              <div
                ...
                onClick={() => navigate(`/counsellor/client-profile/${item.userId}`, {
                  state: { appointmentData: item, clientData: item.userData }
                })}
              >
                {/* Client Avatar */}
                ...
                <div className='flex-1 min-w-0'>
                  <div className='flex items-center gap-2 mb-1'>
                    <p className='font-semibold text-gray-900 truncate'>{item.userData?.name || 'Unknown Client'}</p>
                    {item.payment && (
                  ...
                  <div className='flex items-center gap-4 text-sm text-gray-600'>
                    <span className='flex items-center gap-1'>
                      📅 {formattedDateUniversal(item.slotDate)}
                    </span>
                    <span className='flex items-center gap-1'>
                      ⏰ {item.slotTime}
                    </span>
